# Occam's Razor ↔ Dependency Inversion Principle
...
Both affect whether volatility evidence justifies an abstract port.

## Interaction mechanism

The screened hypothesis was: Occam rejects a policy/detail contract based only on an imagined provider, but accepts it when current volatility or isolation evidence explains the entity. Targeted research did not support it as a general interaction beyond project-specific circumstances.
...
**Candidate hypothesis:** Occam rejects a policy/detail contract based only on an imagined provider, but accepts it when current volatility or isolation evidence explains the entity.

**Observed boundary:** pass
...
Blind primary screen: **Independence / reject**. Blind independent screen: **Moderation / uncertain**. The research pass rejected the hypothesis rather than averaging the two screens.
