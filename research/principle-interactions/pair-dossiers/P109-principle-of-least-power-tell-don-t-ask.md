# Principle of Least Power ↔ Tell, Don't Ask
...
Both affect capability exposed by a state owner.

## Interaction mechanism

The screened hypothesis was: A meaningful command grants the caller only the valid transition instead of broad getters and setters that permit arbitrary mutation sequences. Targeted research did not support it as a general interaction beyond project-specific circumstances.
...
**Candidate hypothesis:** A meaningful command grants the caller only the valid transition instead of broad getters and setters that permit arbitrary mutation sequences.
